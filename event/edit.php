...
<?php if(empty($targetInfo) && empty($postData['organizer_name'])){ // 会員番号と主催者名が入力されていない場合 ?>
    <form method="POST" name="event_target_input" action="./edit.php">
        <div class="row mb-1">
            ...
            <div class="input col-sm-3 themed-grid-col">
                <input class="form-control" type="text" name="member_code" id="member_code" placeholder="000000" maxlength="6" required>
            </div>
        </div>
...
<?php if(!empty($targetInfo) || !empty($postData['organizer_name'])){ // 会員番号か主催者名が入力された場合 ?>
    <form method="POST" name="event_detail_input" action="./edit.php">
        <div class="row mb-1">
            <div class="label col-sm-4 themed-grid-col text-sm-end fw-bold">
                ID
            </div>
            <div class="input col-8 themed-grid-col">
                <?php print($postData['id']); ?>
                <input type="hidden" id="id" name="id" value="<?php print($postData['id']); ?>">
            </div>
        </div>
...
<?php   }else{ ?>
        <?php makeInputText('主催者','organizer_name',$postData,$errMsg,TRUE); ?>
<?php   } ?>
...
<?php makeInputToggle('アンケート','questionnaire',$postData,$errMsg,$toggleButtonValue); ?>
        ...
        <input class="" type="hidden" name="is_update" id="is_update" value="true">
